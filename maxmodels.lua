...
local discovered_items_priority = {}
local bad_items = {}
local ids = {}
...
local item_patterns = {
  ["^https?://www%.maxmodels%.pl/(.+%.html)$"] = "page",
  ["^https?://m?%.?maxmodels%.pl/(.+%.html)$"] = "page",
  ["^https?://www%.maxmodels%.pl/(.+/)$"] = "page",
  ["^https?://m?%.?maxmodels%.pl/(.+/)$"] = "page",
  ["^https?://www%.maxmodels%.pl/(newsroom[^#]*)$"] = "page",
  ["^https?://m?%.?maxmodels%.pl/(newsroom[^#]*)$"] = "page",
  ["^https?://(static%.maxmodels%.pl/.+)$"] = "asset",
  ["^https?://(img%.maxmodels%.pl/.+)$"] = "asset"
}
...
allowed = function(url, parenturl)
  ...
  for pattern, type_ in pairs(item_patterns) do
    ...
    if match then
      ...
      if new_item ~= item_name then
        ...
        if type_ == "asset" then
          target = discovered_items_priority
        end
        discover_item(target, new_item)
        skip = true
      end
    end
  end
  ...
  if not string.match(url, "^https?://[^/]*maxmodels%.pl/") then
    discover_item(discovered_outlinks, string.match(percent_encode_url(url), "^([^%s]+)"))
    return false
  end

  local html = string.match(url, "^https?://[^/]+/(.+%.html)")
    or string.match(url, "^https?://[^/]+/([^/]+)")
    or string.match(url, "^https?://[^/]+/(newsroom[^#]*)")
  if html and ids[string.lower(html)] then
    return true
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url)
    ...
    html = read_file(file)
    local path = string.match(url, "^https?://www%.maxmodels%.pl/(.+)")
      or string.match(url, "^https?://m?%.?maxmodels%.pl/(.+)")
    if path then
      check("https://www.maxmodels.pl/" .. path)
      check("https://m.maxmodels.pl/" .. path)
    end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["maxmodels-rexyc6xnbrzcdcoa"] = discovered_items,
    ["maxmodels-priority-g81g3e0ar4eu0xbf"] = discovered_items_priority,
    ["urls-rz97i6rw0a6929kw"] = discovered_outlinks
  }) do
