...
class PrepareDirectories(SimpleTask):
    ...
    def process(self, item):
        ...
        open('%(item_dir)s/%(warc_file_base)s.warc.zg' % item, 'w').close()
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'maxmodels.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'page':
                url = 'https://www.maxmodels.pl/' + item_value
                wget_args.extend(['--warc-header', 'maxmodels-page: '+url])
                wget_args.append(url)
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'maxmodels-asset: '+url])
                wget_args.append(url)
            else:
